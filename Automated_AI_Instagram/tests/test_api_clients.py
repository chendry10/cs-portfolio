...
import tempfile
import base64
from openai import OpenAI, APIStatusError, AuthenticationError # Import directly
import requests
import json

import Automated_AI_Instagram.src.config as config
from Automated_AI_Instagram.src.api_clients import get_page_id, get_ig_user_id, get, get_meme_prompt_and_caption, get_meme_prompt_via_ai, gen_gpt_image_to_file, _try_responses, upload_to_catbox, upload_with_fallbacks, post_graph_with_retry, post, post_image_with_rehosts # Import new functions
...
def test_get_ig_user_id_success(monkeypatch, caplog):
    ...
    with patch('Automated_AI_Instagram.src.api_clients.get', return_value=mock_response) as mock_get:
        with caplog.at_level(logging.INFO):
            result = get_ig_user_id(mock_page_id)

    mock_get.assert_called_once_with(f"/{mock_page_id}", fields="instagram_business_account{id,username}")
...
def test_get_ig_user_id_no_instagram_business_account(monkeypatch, caplog):
    ...
    mock_get.assert_called_once_with(f"/{mock_page_id}", fields="instagram_business_account{id,username}")
    assert "ERROR: Page found but no 'instagram_business_account' is linked or visible." in str(excinfo.value)

def test_get_ig_user_id_instagram_business_account_no_id(monkeypatch, caplog):
    mock_page_id = "test_page_id"
    mock_response = {
        "instagram_business_account": {
            "username": "test_username_no_id"
        }
    }
...
def test_get_meme_prompt_and_caption_success(caplog):
    ...
    with patch('Automated_AI_Instagram.src.api_clients.get_meme_prompt_via_ai', return_value=mock_raw_output) as mock_get_prompt:
...
def test_get_meme_prompt_and_caption_no_caption(caplog):
    ...
    assert "Could not parse a 'Caption:' from model output. Using a default." in caplog.text
...
def test_gen_gpt_image_to_file_api_status_error_moderation_blocked(mock_images_generate, caplog):
    mock_images_generate.side_effect = [
        APIStatusError("moderation_blocked", response=MagicMock(status_code=400), body={}),
        APIStatusError("moderation_blocked", response=MagicMock(status_code=400), body={}),
        APIStatusError("moderation_blocked", response=MagicMock(status_code=400), body={}),
        APIStatusError("moderation_blocked", response=MagicMock(status_code=400), body={}),
        MagicMock(data=[MagicMock(b64_json=base64.b64encode(b"final_image").decode('utf-8'))]) # Success on 5th attempt
    ]
...
def test_gen_gpt_image_to_file_api_status_error_other(mock_images_generate):
    mock_images_generate.side_effect = APIStatusError("some other error", response=MagicMock(status_code=500), body={})

    prompt = "a test image prompt"
    with pytest.raises(RuntimeError, match="OpenAI Image API call failed: some other error"):
...
def test_gen_gpt_image_to_file_general_exception(mock_images_generate):
    ...
    prompt = "a test image prompt"
    with pytest.raises(RuntimeError, match="OpenAI Image API call failed: network error"):
        gen_gpt_image_to_file(prompt)
...
def test_try_responses_success(mock_openai_client):
    mock_openai_client.responses.create.return_value = MagicMock(output_text="Test response text")
    result = _try_responses(None, "model", "system", "user", openai_client=mock_openai_client)
...
def test_get_meme_prompt_via_ai_success(monkeypatch):
    ...
    with patch('Automated_AI_Instagram.src.api_clients._try_responses', return_value=mock_response) as mock_try_responses:
        result = get_meme_prompt_via_ai("test request")
    mock_try_responses.assert_called_once_with(None, config.PRIMARY_TEXT_MODEL, "You are a helpful assistant that generates meme prompts for use in image generation for instagram.Ensure that the prompt specifies that white meme text should be used on the top and bottom of the image away from the border to avoid being cutoff.", "test request")
    assert result == mock_response

def test_get_meme_prompt_via_ai_no_response(monkeypatch):
    with patch('Automated_AI_Instagram.src.api_clients._try_responses', return_value=None) as mock_try_responses:
        with pytest.raises(RuntimeError, match="Failed to generate prompt with primary model"):
...
def test_upload_to_catbox_success(tmp_path):
    ...
    with patch('Automated_AI_Instagram.src.api_clients.SESSION.post', return_value=mock_response) as mock_post:
...
def test_upload_to_catbox_network_error(tmp_path):
    ...
    with patch('Automated_AI_Instagram.src.api_clients.SESSION.post', side_effect=requests.exceptions.RequestException("Network error")) as mock_post:
        with pytest.raises(RuntimeError, match="Catbox upload failed after 3 tries: Network error"):
...
def test_upload_to_catbox_unexpected_response(tmp_path):
    ...
    with patch('Automated_AI_Instagram.src.api_clients.SESSION.post', return_value=mock_response) as mock_post:
        with pytest.raises(RuntimeError, match="Catbox upload failed after 3 tries: Unexpected Catbox response"):
            upload_to_catbox(str(mock_file_path))
        assert mock_post.call_count == 3 # Retries
...
def test_upload_with_fallbacks_catbox_fails(tmp_path, caplog):
    ...
    with patch('Automated_AI_Instagram.src.api_clients.upload_to_catbox', side_effect=RuntimeError("Catbox failed")) as mock_catbox:
        with patch('Automated_AI_Instagram.src.api_clients.ensure_url_fetchable', return_value=None) as mock_ensure_fetchable:
...
def test_get_success(monkeypatch):
    ...
    with patch('Automated_AI_Instagram.src.api_clients.SESSION.get', return_value=mock_response) as mock_get_session:
...
def test_post_graph_with_retry_5xx_error_and_retry(monkeypatch, caplog):
    ...
    with patch('Automated_AI_Instagram.src.api_clients.SESSION.post', side_effect=[mock_response_500, mock_response_success]) as mock_post_session:
...
def test_post_graph_with_retry_4xx_error_no_retry(monkeypatch, caplog):
    ...
    mock_response_400.json.return_value = {"error": {"message": "Bad Request", "code": 400}}
    ...
    with patch('Automated_AI_Instagram.src.api_clients.SESSION.post', return_value=mock_response_400) as mock_post_session:
        with patch('time.sleep', return_value=None) as mock_sleep:
            with caplog.at_level(logging.ERROR):
...
def test_post_graph_with_retry_non_json_response(monkeypatch, caplog):
    ...
    with patch('Automated_AI_Instagram.src.api_clients.SESSION.post', return_value=mock_response) as mock_post_session:
        with patch('time.sleep', return_value=None) as mock_sleep:
            with pytest.raises(RuntimeError, match="Non-JSON response: 200, text: Not JSON content"):
                post_graph_with_retry("/test_url", {"data": "some_data"})
            assert mock_post_session.call_count == 1
